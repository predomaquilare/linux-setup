#!/usr/bin/env bash
# =============================================================================
#  01-hyprland.sh — Instala Hyprland + Waybar + dependencias no Ubuntu 22.04
...
clone_or_update https://github.com/hyprwm/hyprland-guiutils    "$SRC_DIR/hyprland-guiutils"
...
# hyprland-guiutils (remove aviso de runtime no Hyprland)
log "Compilando hyprland-guiutils..."
GUIUTILS_BUILD="$SRC_DIR/hyprland-guiutils/build"; rm -rf "$GUIUTILS_BUILD"
cmake -B "$GUIUTILS_BUILD" -S "$SRC_DIR/hyprland-guiutils" \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_C_COMPILER="$CC_BIN" -DCMAKE_CXX_COMPILER="$CXX_BIN" \
      -DCMAKE_CXX_FLAGS="$CXX_FLAGS" \
      -DCMAKE_EXE_LINKER_FLAGS="$LD_FLAGS" \
      -DCMAKE_INSTALL_PREFIX=/usr/local
cmake --build "$GUIUTILS_BUILD" -j"$(nproc)"
sudo cmake --install "$GUIUTILS_BUILD"
sudo ldconfig
ok "hyprland-guiutils instalado."
...
if [[ -f "$CONFIGS_DIR/hypr/hyprland.conf" ]]; then
  cp "$CONFIGS_DIR/hypr/hyprland.conf" "$CONFIG_DIR/hyprland.conf"
  ok "hyprland.conf copiado de configs/hypr/"
else
  warn "configs/hypr/hyprland.conf nao encontrado, gerando padrao..."
