#!/usr/bin/env bash
# =============================================================================
#  04-dotfiles.sh — Aplica configs: tmux, vim, zsh, ranger, kitty, hyprland
#  Puxando arquivos de configs/ quando disponivel, senao gera inline
...
# Usa arquivo externo se existir, senao usa fallback inline
use_config() {
  ...
  return 1  # fallback: caller gera inline
}
...
use_config "tmux/tmux.conf" "$HOME/.tmux.conf" && tmux_done=1 || tmux_done=0
[[ "$tmux_done" -eq 1 ]] && info "tmux.conf copiado de configs/tmux/" || true
[[ "$tmux_done" -eq 0 ]] && {
python3 - << 'PYEOF'
import os
content = r"""# =============================================================================
...
use_config "ranger/rc.conf" "$HOME/.config/ranger/rc.conf" || {
...
use_config "zathura/zathurarc" "$HOME/.config/zathura/zathurarc" || {
...
# -----------------------------------------------------------------------------
# ~/.config/fuzzel/fuzzel.ini — launcher Eva-01
# -----------------------------------------------------------------------------
info "Configurando fuzzel..."
mkdir -p "$HOME/.config/fuzzel"
cat > "$HOME/.config/fuzzel/fuzzel.ini" << 'FUZZEL_EOF'
[main]
