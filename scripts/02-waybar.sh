#!/usr/bin/env bash
# =============================================================================
#  02-waybar.sh — Instala tema Eva-01 no Waybar
...
if [[ -f "$CONFIGS_DIR/waybar/config.jsonc" ]]; then
  cp "$CONFIGS_DIR/waybar/config.jsonc" "$WAYBAR_DIR/config.jsonc"
  ok "config.jsonc copiado de configs/waybar/"
else
  warn "configs/waybar/config.jsonc nao encontrado, gerando padrao..."
...
if [[ -f "$CONFIGS_DIR/waybar/style.css" ]]; then
  cp "$CONFIGS_DIR/waybar/style.css" "$WAYBAR_DIR/style.css"
  ok "style.css copiado de configs/waybar/"
else
  warn "configs/waybar/style.css nao encontrado, gerando padrao..."
